...
object OpenReadingFrames extends App {

  Challenge("OpenReadingFrames.fasta") provides { data =>

    Challenge("DNACodonsTable.txt") provides { codons =>

      // Pattern match codon + corresponding letter or "stop"
      val pattern = new Regex("([ATGC]{3}) ([a-zA-Z]+)")

      // Find all matches of these two groups
      val allMatches = pattern.findAllMatchIn(codons.mkString)
